export default function database(name, stores, version = 1) {
  let dbFuture;
...
function store(name, { execute }) {
  ...
  function put(key, item) {
    return execute(db => do_put(db, name, key, item));
  }

  function get(key) {
    return execute(db => do_get(db, name, key));
  }

  function getAll() {
    return execute(db => do_getAll(db, name));
  }

  function getAllKeys() {
    return execute(db => do_getAllKeys(db, name));
  }

  function update(key, item) {
    return execute(db => do_update(db, name, key, item));
  }

  function count(key) {
    return execute(db => do_count(db, name, key));
  }

  function remove(key) {
    return execute(db => do_delete(db, name, key));
  }

  function clear() {
    return execute(db => do_clear(db, name));
  }
}
...
function do_count(db, store, key) {
  const request = db.transaction(store).objectStore(store).count(key);
